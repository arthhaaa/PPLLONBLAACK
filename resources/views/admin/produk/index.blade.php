...
@section('content')
@php
    $stockMeta = function ($stock) {
        $stock = (int) $stock;

        if ($stock <= 0) {
            return ['class' => 'is-empty', 'label' => 'Habis'];
        }

        if ($stock <= 10) {
            return ['class' => 'is-low', 'label' => 'Menipis'];
        }

        return ['class' => 'is-ready', 'label' => 'Tersedia'];
    };
@endphp
...
<div class="product-admin-page container-fluid px-4 pb-5">
    ...
    <div class="row g-4">
        ...
        @php($stock = $stockMeta($product->stok_produk))
